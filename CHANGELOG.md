...
- Updated dependencies:
  - `orion-error` from `0.7` to `0.8`
  - `orion-variate` from `0.12` to `0.13`
- `ConfIOReason` now uses `#[derive(OrionError)]` instead of `thiserror::Error` + manual trait impls
- Removed manual `DomainReason`, `ErrorCode`, `ErrorIdentityProvider` implementations (generated by derive)
- `UvsReason` → `UnifiedReason`, `Uvs(` variant → `General(`
- Removed legacy `compat_traits::ErrorOwe`, `UvsFrom`, `UvsConfFrom` re-exports (removed in orion-error 0.8)
- `owe_res()` → `source_err(ConfIOReason::system_error(), ...)`
- `traits_ext::ToStructError` → `conversion::ToStructError`
- `ErrorWith` import moved from root to `conversion::ErrorWith`
- `ctx.record("from path", path)` → `ctx.record("from path", path.display().to_string())`
- Removed `thiserror` dependency (replaced by OrionError derive)

### Added
- `ConfIOReason::system_error()` and other delegate constructors via `#[orion_error(transparent)]` for `General(UnifiedReason)` variant
- Re-exported `ConfIOReason`, `OrionConfResult`, `OrionConfError` from crate root

### Fixed
- Fixed compile errors caused by orion-error 0.8 API changes
- Fixed `Path` does not implement `Display` error by using `path.display()`
